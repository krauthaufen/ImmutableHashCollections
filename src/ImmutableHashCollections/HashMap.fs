...
module internal HashMapListAdaptive =
...
type HashMapAdaptive<'K, [<EqualityConditionalOn>] 'V> internal(cnt: int, store: intmap<list<struct ('K * 'V)>>) =
    static let empty = HashMapAdaptive<'K, 'V>(0, IntMap.empty)
    ...
    member x.Alter (key: 'K, update: option<'V> -> option<'V>) =
        ...
        let update (old: option<list<struct('K * 'V)>>) =
            ...
            | None -> 
                ...
                | None -> 
                    None
            | Some oldList ->
                match HashMapListAdaptive.tryRemove key oldList with
                | Some (oldValue, rest) ->
        ...
        if changed then HashMapAdaptive(cnt + deltaCnt, newStore)
        else x
        ...
        let newMap = 
            store |> IntMap.alter (function 
                | None -> 
                    ...
                    Some [key,value]
                | Some l -> 
                    Some (HashMapListAdaptive.add cnt key value l)
            ) hash
        HashMapAdaptive(!cnt, newMap)
    ...
    member x.Remove (key: 'K) =
        ...
        let newMap = 
            store |> IntMap.update (fun l ->
                match HashMapListAdaptive.remove cnt key l with
                | [] -> None
                | l -> Some l
            ) hash
        HashMapAdaptive(!cnt, newMap)
    ...
    member x.ChooseTup(mapping: 'K -> 'V -> option<'V2 * 'V3>) =
        ...
        HashMapAdaptive(cnt, a), HashMapAdaptive(cnt, b)
    ...
    /// `O(N + M)`
    member x.UnionWith(other: HashMapAdaptive<'K, 'V>, collide: 'K -> 'V -> 'V -> 'V) =
        ...
        let newStore = 
            IntMap.appendWith (HashMapListAdaptive.unionWith f) store other.Store
        
        HashMapAdaptive(cnt, newStore)
        
    /// Like choose2 but with existing right values.
    member x.UpdateTo(other : HashMapAdaptive<'K, 'V2>, mapping : 'K -> option<'V> -> 'V2 -> 'V3) =
    ...
    /// `O(N + M)`
    member x.Choose2(other: HashMapAdaptive<'K, 'V2>, mapping: 'K -> option<'V> -> option<'V2> -> option<'V3>) =
        let mutable cnt = 0
        let f k l r =
        ...
        let both (_hash: int) (l: list<struct ('K * 'V)>) (r: list<struct ('K * 'V2)>) =
            match HashMapListAdaptive.mergeWithOption f l r with
            | [] -> None
            | l -> Some l
    ...
    /// Note that one of the options will always be some.
    /// `O(N + M)`
    member x.Map2(other: HashMapAdaptive<'K, 'V2>, f: 'K -> option<'V> -> option<'V2> -> 'V3) =
        let mutable cnt = 0
        let f k l r =
            cnt <- cnt + 1
            f k l r
        ...
        let both (hash: int) (l: list<struct ('K * 'V)>) (r: list<struct ('K * 'V2)>) =
            match HashMapListAdaptive.mergeWith f l r with
            | [] -> None
            | l -> Some l
    ...
    /// `O(N + M)`
    member x.Union(other: HashMapAdaptive<'K, 'V>) =
    ...
    member x.TryRemove(key: 'K) =
        ...
        let newStore =
            store |> IntMap.update (fun o ->
                match HashMapListAdaptive.tryRemove key o with
                | Some(v,l) ->
                    removed <- Some v
                    match l with
                    | [] -> None
        ...
        match removed with
        | Some rem -> Some(rem, HashMapAdaptive(cnt - 1, newStore))
        | None -> None
    ...
    static member Single (k: 'K) (v: 'V) =
        ...
        HashMapAdaptive(1, IntMap.single hash [(k, v)])
    ...
    /// `O(1)`
    member x.ConservativeEquals(other : HashMapAdaptive<'K, 'V>) =
    ...
    override x.GetHashCode() =
        ...
        | _ -> 
            (0, store) ||> Seq.fold (fun s (h,vs) ->
                ...
                HashMapListAdaptive.combineHash h listHash
            )
    ...
    override x.Equals o =
        ...
        match o with
        | :? HashMapAdaptive<'K, 'V> as o ->
            IntMap.equals HashMapListAdaptive.equals store o.Store
        | _ ->
            false
        #endif
...
module HashMapAdaptive =

    /// The empty map.
    [<GeneralizableValue>]
    let empty<'K, 'V> = HashMapAdaptive<'K, 'V>.Empty
    
    /// Creates a map with a single entry.
    /// `O(1)`
    let inline single (k: 'K) (v: 'V) = 
        HashMapAdaptive.Single k v

    /// Creates a map with all entries from the seq.
    /// `O(N * log N)`
    let inline ofSeq (seq: seq<'K * 'V>) = 
        HashMapAdaptive.OfSeq seq
    ...
    let inline ofList (list: list<'K * 'V>) = 
        HashMapAdaptive.OfList list
        
    /// Creates a map with all entries from the array.
    /// `O(N * log N)`
    let inline ofArray (arr: array<'K * 'V>) = 
        HashMapAdaptive.OfArray arr

    
    /// Creates a seq holding all tuples contained in the map.
    /// `O(N)`
    let inline toSeq (map: HashMapAdaptive<'K, 'V>) = 
        map.ToSeq()

    /// Creates a list holding all tuples contained in the map.
    /// `O(N)`
    let inline toList (map: HashMapAdaptive<'K, 'V>) = 
        map.ToList()

    /// Creates a list holding all tuples contained in the map.
    /// `O(N)`
    let inline toArray (map: HashMapAdaptive<'K, 'V>) = 
        map.ToArray()

    /// Creates a Map holding all entries contained in the HashMap.
    /// `O(N)`
    let inline toMap (map: HashMapAdaptive<'K, 'V>) = 
    ...
    /// Adds or updates the entry for the given key. `O(log N)`
    let inline add (key: 'K) (value: 'V) (map: HashMapAdaptive<'K, 'V>) =
        map.Add(key, value)

    /// Removes the entry for the given key. `O(log N)`
    let inline remove (key: 'K) (map: HashMapAdaptive<'K, 'V>) =
        map.Remove(key)
    ...
    /// A new value (or None for deleting the entry).
    /// `O(log N)`
    let inline alter (key: 'K) (mapping: option<'V> -> option<'V>) (map: HashMapAdaptive<'K, 'V>) =
        map.Alter(key, mapping)
        
    /// Adds or updates the entry for the given key based on the optional current value. 
    /// `O(log N)`
    let inline update (key: 'K) (mapping: option<'V> -> 'V) (map: HashMapAdaptive<'K, 'V>) =
    ...
    /// `O(N + M)`
    let inline unionWith (resolve: 'K -> 'V -> 'V -> 'V) (l: HashMapAdaptive<'K, 'V>) (r: HashMapAdaptive<'K, 'V>) =
        l.UnionWith(r, resolve)
        
    /// Creates a new map containing all elements from l and r.
    /// Colliding entries are taken from r.
    /// `O(N + M)`
    let inline union (l: HashMapAdaptive<'K, 'V>) (r: HashMapAdaptive<'K, 'V>) =
        l.Union r

    /// Tries to remove the entry for the given key from the map and returns its value and the rest of the map.
    /// `O(log N)`
    let inline tryRemove (key: 'K) (map: HashMapAdaptive<'K, 'V>) =
        map.TryRemove key


    /// Creates a new map (with the same keys) by applying the given function to all entries.
    /// `O(N)`
    let inline map (mapping: 'K -> 'V -> 'V2) (map: HashMapAdaptive<'K, 'V>) =
        map.Map(mapping)
        
    /// Creates a new map (with the same keys) by applying the given function to all entries.
    /// `O(N)`
    let inline choose (mapping: 'K -> 'V -> option<'V2>) (map: HashMapAdaptive<'K, 'V>) =
        map.Choose mapping

    /// Creates a new map (with the same keys) that contains all entries for which predicate was true.
    /// `O(N)`
    let inline filter (predicate: 'K -> 'V -> bool) (map: HashMapAdaptive<'K, 'V>) =
        map.Filter predicate

    /// Applies the iter function to all entries of the map.
    /// `O(N)`
    let inline iter (iter: 'K -> 'V -> unit) (map: HashMapAdaptive<'K, 'V>) =
        map.Iter iter

    /// Folds over all entries of the map.
    /// Note that the order for elements is undefined.
    /// `O(N)`
    let inline fold (folder: 'State -> 'K -> 'V -> 'State) (seed: 'State) (map: HashMapAdaptive<'K, 'V>) =
        map.Fold(seed, folder)
        
    /// Tests whether an entry making the predicate true exists.
    /// `O(N)`
    let inline exists (predicate: 'K -> 'V -> bool) (map: HashMapAdaptive<'K, 'V>) =
        map.Exists(predicate)

    /// Tests whether all entries fulfil the given predicate.
    /// `O(N)`
    let inline forall (predicate: 'K -> 'V -> bool) (map: HashMapAdaptive<'K, 'V>) =
    ...
    /// Note that one of the options will always be some.
    /// `O(N + M)`
    let inline map2 (mapping: 'K -> option<'V> -> option<'V2> -> 'V3) (l: HashMapAdaptive<'K, 'V>) (r: HashMapAdaptive<'K, 'V2>) =
        l.Map2(r, mapping)
    ...
    /// Note that one of the options will always be some.
    /// `O(N + M)`
    let inline choose2 (mapping: 'K -> option<'V1> -> option<'V2> -> option<'V3>) (l: HashMapAdaptive<'K, 'V1>) (r: HashMapAdaptive<'K, 'V2>) =
        l.Choose2(r, mapping)

    /// Tries to find the value for the given key.
    /// `O(log N)`
    let inline tryFind (key: 'K) (map: HashMapAdaptive<'K, 'V>) =
        map.TryFind key
        
    /// Finds the value for the given key and raises KeyNotFoundException on failure.
    /// `O(log N)`
    let inline find (key: 'K) (map: HashMapAdaptive<'K, 'V>) =
        map.Find key
        
    /// Tests if an entry for the given key exists. `O(log N)`
    let inline containsKey (key: 'K) (map: HashMapAdaptive<'K, 'V>) =
        map.ContainsKey key

    /// The number of elements in the map `O(1)`
    let inline count (map: HashMapAdaptive<'K, 'V>) = map.Count
    
    /// Is the map empty? `O(1)`
    let inline isEmpty (map: HashMapAdaptive<'K, 'V>) = map.IsEmpty


    /// Determines the operations needed to transform l into r.
    /// Returns a HashMapDelta containing all the needed operations.
    let computeDelta (l: HashMapAdaptive<'A, 'B>) (r: HashMapAdaptive<'A, 'B>): HashMapAdaptive<'A, ElementOperation<'B>> =
